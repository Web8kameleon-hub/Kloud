...
async fn main() {
    ...
    let transport = protocol::QuicTransport::new(&format!("0.0.0.0:{}", listen_port))
        .await
        .expect("QUIC endpoint failed");

    // QUIC metrics (placeholder - extend QuicTransport)
    let transport_metrics = Arc::new(Mutex::new(protocol::TransportMetrics::default()));

    // Start QUIC listener for gossip channels
    let transport_clone = transport.clone();
    let digest_tx_clone = digest_tx.clone();
    let request_tx_clone = request_tx.clone();
    let payload_tx_clone = payload_tx.clone();
    tokio::spawn(async move {
        transport_clone.start_listener(|data, stream_id| {
            // Route by stream_id: 0=digest, 1=request, 2=payload
            match stream_id % 3 {
                0 => { let _ = digest_tx_clone.try_send(serde_cbor::from_slice(&data).ok().unwrap()); },
                1 => { let _ = request_tx_clone.try_send(serde_cbor::from_slice(&data).ok().unwrap()); },
                2 => { let _ = payload_tx_clone.try_send(serde_cbor::from_slice(&data).ok().unwrap()); },
                _ => {},
            }
        });
    });
    ...
    let gossip_handle = tokio::spawn(async move {
        let gossip_engine = Arc::clone(&gossip_engine);
        loop {
    ...
    let interface_handle = tokio::spawn(async move {
        let merge_sync = merge_sync_arc.clone();
