"""
═══════════════════════════════════════════════════════════════════════════════
CLX ADAPTER - Implementon kontratën e motorit për CLX
═══════════════════════════════════════════════════════════════════════════════

Ky adapter map-on kontratën unike të Kloud në API-n e CLX.
...
CLX_HOST = "host.docker.internal" if IS_IN_DOCKER else "localhost"
CLX_BASE_URL = os.environ.get(
    "CLX_URL",
    os.environ.get("CLX_BASE_URL", f"http://{CLX_HOST}:11434"),
)
DEFAULT_MODEL = os.environ.get("CLX_MODEL", "clx")
...
class ClxAdapter(BaseEngineAdapter):
    """
    Adapter për CLX - LLM lokal

    Përdorimi:
        adapter = ClxAdapter("clx:clx")
    ...
    def __init__(
        self,
        engine_id: str = "clx:default",
        model: str = DEFAULT_MODEL,
        base_url: str = CLX_BASE_URL,
        timeout: float = DEFAULT_TIMEOUT,
        config: Optional[Dict[str, Any]] = None,
    ):
        super().__init__(engine_id, config or {})
        ...
        self.supported_modes = [
            EngineMode.CHAT,
            EngineMode.COMPLETION,
            EngineMode.EMBEDDING,
        ]
...
"""
    ...
    async def initialize(self) -> bool:
        """Inicializon adapterin dhe kontrollon lidhjen me CLX"""
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"✅ CLX connected: {len(self._available_models)} models available"
                )
                return True
            else:
                logger.warning(f"CLX connection failed: {response.status_code}")
                return False

        except Exception as e:
            logger.error(f"CLX initialization error: {e}")
            return False
    ...
    async def _generate_chat(self, request: EngineRequest) -> EngineResponse:
        ...
        if request.params.seed:
            clx_request["options"]["seed"] = request.params.seed

        try:
            client = self._client
            if client is None:
                raise RuntimeError("CLX client not initialized")
            ...
            if response.status_code != 200:
                return EngineResponse(
                    ...
                    engine_id=self.engine_id,
                    success=False,
                    error=f"CLX error: {response.status_code} - {response.text}",
                )
            ...
            logger.error(f"CLX chat error: {e}")
            return EngineResponse(
                text="",
                engine_id=self.engine_id,
                success=False,
        ...
        try:
            ...
            if response.status_code != 200:
                return EngineResponse(
                    ...
                    engine_id=self.engine_id,
                    success=False,
                    error=f"CLX error: {response.status_code}",
                )
            ...
            logger.error(f"CLX completion error: {e}")
            return EngineResponse(
                text="",
                engine_id=self.engine_id,
                success=False,
        ...
        try:
            ...
            if response.status_code != 200:
                return EngineResponse(
                    ...
                    engine_id=self.engine_id,
                    success=False,
                    error=f"CLX embedding error: {response.status_code}",
                )
            ...
            logger.error(f"CLX embedding error: {e}")
            return EngineResponse(
                text="",
                engine_id=self.engine_id,
                success=False,
...
def create_clx_adapter(
    model: str = DEFAULT_MODEL,
    engine_id: Optional[str] = None,
) -> ClxAdapter:
    """Krijon CLX adapter"""
    eid = engine_id or f"clx:{model}"
    return ClxAdapter(engine_id=eid, model=model)


async def test_clx_connection() -> bool:
    """Teston lidhjen me CLX"""
    adapter = ClxAdapter()
