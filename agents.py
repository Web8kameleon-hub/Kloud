    ...
    async def _analyze_data(self, payload: Dict) -> Dict:
        ...
        return {
            "analysis": {
                "patterns_found": 3,
                "correlations": [
                    {"field_a": "cpu", "field_b": "memory", "correlation": 0.85}
                ],
    ...
    async def _recommend(self, payload: Dict) -> Dict:
        ...
        return {
            "recommendations": [
                {
                    "priority": "medium",
                    "action": "Consider scaling database",
                    "impact": "high",
                },
                {
                    "priority": "low",
                    "action": "Enable caching for static assets",
                    "impact": "medium",
                },
            ],
        ...
        with self._lock:
            ...
            for agent in self._agents:
                if agent.status == AgentStatus.READY:
                    ...
                    logger.info(
                        f"📉 Removed instance: {agent.id} (total: {len(self._agents)})"
                    )
                    return True
        return False
...
class AgentRegistry:
    ...
    def register(
        self, agent_class: type, min_instances: int = 1, max_instances: int = 5
    ) -> str:
...
class AgentOrchestrator:
    ...
    async def broadcast(
        self, payload: Dict[str, Any], agent_names: Optional[List[str]] = None
    ) -> Dict[str, TaskResult]:
        ...
        tasks: List[Awaitable[TaskResult]] = []
        ...
        for name, result in zip(agent_names, task_results):
            if isinstance(result, BaseException):
                results[name] = TaskResult(
                    task_id="unknown", agent_id=name, success=False, error=str(result)
                )
            else:
                results[name] = result
...
async def demo():
    ...
    for agent in orchestrator.list_agents():
        print(
            f"  • {agent['name']} ({agent['config']['agent_type']}) - {agent['pool']['total_instances']} instances"
        )
    ...
    result = await orchestrator.submit("alba", {"action": "collect"})
    print(
        f"  ALBA result: success={result.success}, duration={result.duration_ms:.2f}ms"
    )

    # Task to ALBI
    result = await orchestrator.submit("albi", {"action": "detect_anomaly"})
    print(
        f"  ALBI result: success={result.success}, duration={result.duration_ms:.2f}ms"
    )

    # Task to JONA
    result = await orchestrator.submit("jona", {"action": "recommend"})
    print(
        f"  JONA result: success={result.success}, duration={result.duration_ms:.2f}ms"
    )
