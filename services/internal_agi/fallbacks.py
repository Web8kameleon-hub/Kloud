...
class FallbackManager:
    ...
    def build_fallback_response(
        ...
        """
        Build error response when sources fail (no fallback synthesis)

        Args:
            query: Original user query
            failed_sources: Sources that failed
            working_sources: Sources that succeeded
            partial_data: Unused - error only response

        Returns:
            FallbackResponse signaling error state
        """
        # Determine error level
        total = failed_sources + working_sources
        level = self.get_fallback_level(failed_sources, total)

        # Always FAIL action - no fallback synthesis
        action = FallbackAction.FAIL

        # Error response - no synthetic templates
        answer = f"Unable to process query '{query}' - upstream services unavailable"
        explanation = f"Failed sources: {len(failed_sources)} / {len(total)}"
        suggestions = ["Please retry later", "Check backend status"]
        ...
        return FallbackResponse(
            ...
            recovered_data={},  # No recovered/cached fallback data
        )
    ...
    def _find_cached_fallback(self, query: str) -> Dict[str, Any]:
        ...
        if not result:
            result["general"] = (
                "Kloud Internal AGI - 4053+ sources across 155+ countries"
            )
    ...
    def get_alternative_sources(self, failed_source: str) -> List[str]:
        ...
        alternatives = {
            ...
            "americas_sources": [
                "global_data_sources",
                "caribbean_central_america_sources",
            ],
            "asia_china_sources": [
                "global_data_sources",
                "asia_oceania_global_sources",
            ],
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        circuits_open = [
            sid for sid, h in self._source_health.items() if h.circuit_open
        ]
