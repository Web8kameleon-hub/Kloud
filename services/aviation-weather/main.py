...
def parse_flight_category(metar_raw: str) -> FlightConditions:
    ...
    categories = {
        "VFR": {
            "color": "#00C853",
            "description": "Visual Flight Rules - Good conditions",
        },
        "MVFR": {
            "color": "#2196F3",
            "description": "Marginal VFR - Reduced visibility",
        },
        "IFR": {
            "color": "#F44336",
            "description": "Instrument Flight Rules - Poor conditions",
        },
    ...
    return FlightConditions(
        category=category, color=cat_info["color"], description=cat_info["description"]
    )
...
async def get_metar(icao: str):
    ...
    if len(icao) != 4:
        raise HTTPException(
            status_code=400, detail="Invalid ICAO code - must be 4 characters"
        )
...
async def get_multi_airport(
    ...
    for i, code in enumerate(codes):
        ...
        if isinstance(metar, str) and metar:
            ...
            results.append({"icao": code, "error": "Not available"})
...
async def search_airports(
    query: str = Query(
        ..., min_length=2, description="Search by ICAO, IATA, or city name"
    ),
):
    """Search for airports"""
