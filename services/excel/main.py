...
from fluid_runtime import activate_wwwmmm_fluid
...
async def generate_excel(
    title: str = "Kloud Data Export", include_metrics: bool = True
):
    """Generate Excel file with system data"""
    if not EXCEL_AVAILABLE or Workbook is None or Font is None or PatternFill is None:
        return JSONResponse(
            status_code=503,
            content={
    ...
    try:
        ...
        if ws is None:
            raise HTTPException(
                status_code=500, detail="Failed to initialize worksheet"
            )
...
async def validate_excel(file: UploadFile = File(...)):
    ...
    if not EXCEL_AVAILABLE or load_workbook is None:
        return JSONResponse(
            status_code=503, content={"error": "Excel validation not available"}
        )
    ...
    try:
        ...
        for sheet_name in wb.sheetnames:
            ...
            validation["total_columns"] = max(
                validation["total_columns"], ws.max_column
            )
        ...
        return {"filename": file.filename, "valid": False, "error": str(e)}


@app.post("/api/excel/parse")
async def parse_excel(file: UploadFile = File(...), sheet: Optional[str] = None):
    """Parse Excel file and return JSON data"""
    if not EXCEL_AVAILABLE or load_workbook is None:
        return JSONResponse(
            status_code=503, content={"error": "Excel parsing not available"}
        )
    ...
    try:
        ...
        if ws is None:
            raise HTTPException(status_code=500, detail="Failed to select worksheet")
        ...
        for row_idx, row in enumerate(ws.iter_rows(values_only=True)):
            if row_idx == 0:
                headers = [
                    str(cell) if cell else f"Column_{i}" for i, cell in enumerate(row)
                ]
            else:
                row_dict = {}
...
async def get_formulas():
    ...
    return {
        "supported_formulas": [
            {
                "name": "=PY()",
                "description": "Execute Python code in Excel",
                "example": '=PY("import pandas as pd; result = df.sum()")',
            },
            {
                "name": "=KLOUD.STATUS()",
                "description": "Get system status",
                "example": "=KLOUD.STATUS()",
            },
            {
                "name": "=KLOUD.METRICS()",
                "description": "Get live metrics",
                "example": "=KLOUD.METRICS()",
            },
            {
                "name": "=KLOUD.API(endpoint)",
                "description": "Call API endpoint",
                "example": '=KLOUD.API("/api/status")',
            },
        ],
        "office_scripts": [
        ...
        "templates": [
            {
                ...
                "description": "System metrics with charts",
            },
            {
                "id": "sla",
                "name": "SLA Report",
                "description": "SLA tracking and compliance",
            },
            {
                "id": "api-catalog",
                "name": "API Catalog",
                "description": "All 71 API endpoints documented",
            },
            {
                "id": "functions",
                "name": "Functions Registry",
                "description": "Python and Office Script functions",
            },
        ]
    }
