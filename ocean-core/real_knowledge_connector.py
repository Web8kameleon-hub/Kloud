...
class CoinGeckoConnector:
    ...
    def format_response(self, data: Dict[str, Any], query: str) -> str:
        ...
        if "bitcoin" in q_lower or "btc" in q_lower:
            ...
            return (
                f"💰 **Bitcoin (BTC)** aktualisht tregtohet në **${price:,.2f}**. Në 24 orët e fundit ka {direction} me {abs(change):.1f}%."
            )
...
class ConversationalAI:
    ...
    def add_to_context(self, role: str, message: str):
        ...
        self.context_history.append({"role": role, "message": message, "timestamp": datetime.now().isoformat()})
    ...
    def generate_natural_response(self, query: str, data: Dict[str, Any]) -> str:
        ...
        # Add conversational element - ERROR STATE
        if not response_parts:
            response_parts.append("")  # No fallback response, error signals real failure
    ...
    def _generate_fallback_response(self, query: str) -> str:
        """No fallback response - return empty to signal real error state"""
        return ""
...
class RealKnowledgeConnector:
    ...
    async def process_query(self, query: str) -> Dict[str, Any]:
        ...
        """
        q_lower = query.lower()
        response_data = {"query": query, "timestamp": datetime.now(timezone.utc).isoformat(), "sources_used": []}
        ...
        response_data["final_response"] = self.conversational_ai.generate_natural_response(query, response_data)
