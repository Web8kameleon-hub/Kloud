...
class EntityExtractor:
    ...
    async def extract(
        self, text: str, entity_types: Optional[List[EntityType]] = None, use_llm: bool = True, language: str = "auto"
    ...
    def _extract_patterns(self, text: str, entity_types: Optional[List[EntityType]]) -> List[Entity]:
    ...
    def _extract_names(self, text: str) -> List[Entity]:
        ...
        for match in re.finditer(title_pattern, text):
            entities.append(
                Entity(text=match.group(), entity_type=EntityType.PERSON, start_pos=match.start(), end_pos=match.end(), confidence=0.9)
            )
        ...
        for match in re.finditer(name_pattern, text):
            ...
            if match.start() > 0 and text[match.start() - 1] not in ".!?\n":
                entities.append(
                    Entity(text=name, entity_type=EntityType.PERSON, start_pos=match.start(), end_pos=match.end(), confidence=0.7)
                )
    ...
    def _extract_organizations(self, text: str) -> List[Entity]:
        ...
        for match in re.finditer(org_pattern, text):
            entities.append(
                Entity(
                    text=match.group(), entity_type=EntityType.ORGANIZATION, start_pos=match.start(), end_pos=match.end(), confidence=0.9
                )
            )
...
Entities:"""
            ...
            async with httpx.AsyncClient(timeout=15.0) as client:
                response = await client.post(
                    ...
                    json={"model": "llama3.1:8b", "prompt": prompt, "stream": False, "options": {"temperature": 0.2, "num_predict": 300}},
                )
