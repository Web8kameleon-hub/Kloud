...
class SummaryResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "word_count": {"original": self.word_count_original, "summary": self.word_count_summary},
...
class TextSummarizer:
    ...
    async def summarize(
        ...
        return SummaryResult(
            ...
            metadata={"sentence_count": len(sentences), "target_ratio": target_ratio, "summarizer_version": "1.0.0"},
        )
    ...
    async def _abstractive_summarize(self, text: str, language: str, target_ratio: float) -> str:
        """Generate abstractive summary using LLM"""
        target_words = int(len(text.split()) * target_ratio)

        llm_summary = await self._llm_summarize(text, language, f"approximately {target_words} words")
    ...
    async def _llm_summarize(self, text: str, language: str, length_hint: str) -> Optional[str]:
...
Summary:"""
            ...
            async with httpx.AsyncClient(timeout=15.0) as client:
                response = await client.post(
                    ...
                    json={"model": "llama3.1:8b", "prompt": prompt, "stream": False, "options": {"temperature": 0.5, "num_predict": 200}},
                )
    ...
    async def summarize_documents(self, documents: List[str], **kwargs) -> SummaryResult:
