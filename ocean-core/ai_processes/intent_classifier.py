...
class IntentResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"intent": self.intent.value, "confidence": round(self.confidence, 3), "keywords": self.keywords}
...
class IntentClassifier:
    ...
    INTENT_PATTERNS = {
        ...
        UserIntent.FAREWELL: {
            "keywords": ["bye", "goodbye", "see you", "take care", "farewell", "mirupafshim", "lamtumirë", "ditën e mirë"],
        ...
        UserIntent.HELP: {
            "keywords": ["help", "assist", "support", "guidance", "how do i", "can you help", "ndihmë", "si mund"],
            "patterns": [r"\b(help|assist)\s+me", r"can\s+you\s+help", r"how\s+do\s+i"],
        },
        UserIntent.QUERY: {
            "keywords": ["what", "who", "when", "where", "which", "çfarë", "kush", "kur", "ku", "cili"],
            "patterns": [r"^(what|who|when|where|which)\b", r"\?$"],
        },
        UserIntent.EXPLAIN: {
            "keywords": ["explain", "why", "how", "describe", "elaborate", "shpjego", "pse", "si"],
            "patterns": [r"^(explain|why|how)\b", r"can\s+you\s+explain"],
        },
        UserIntent.DEFINE: {
            "keywords": ["define", "definition", "meaning", "what is", "what's", "përcakto", "çfarë është"],
            "patterns": [r"what\s+(is|are)\s+", r"\bdefine\b", r"meaning\s+of"],
        },
        UserIntent.CREATE: {
            "keywords": ["create", "make", "build", "generate", "new", "add", "krijo", "bëj", "shto"],
            "patterns": [r"^(create|make|build|add)\b", r"i\s+want\s+to\s+(create|make)"],
        },
        UserIntent.UPDATE: {
            "keywords": ["update", "modify", "change", "edit", "fix", "correct", "përditëso", "ndrysho"],
            "patterns": [r"^(update|modify|change|edit)\b"],
        },
        UserIntent.DELETE: {"keywords": ["delete", "remove", "clear", "cancel", "fshi", "hiq"], "patterns": [r"^(delete|remove|clear)\b"]},
        UserIntent.SEARCH: {
        ...
        UserIntent.CONFIGURE: {
            "keywords": ["configure", "setup", "set up", "settings", "config", "konfiguro", "rregullime"],
            "patterns": [r"^(configure|setup|set\s+up)\b"],
        },
        UserIntent.EXECUTE: {
            "keywords": ["run", "execute", "start", "launch", "trigger", "ekzekuto", "fillo"],
        ...
        UserIntent.CANCEL: {"keywords": ["cancel", "abort", "stop", "undo", "anulo", "ndalo"], "patterns": [r"^(cancel|abort|stop)\b"]},
        ...
        UserIntent.REPORT_ISSUE: {
            "keywords": ["bug", "issue", "problem", "error", "broken", "not working", "gabim", "problem"],
            "patterns": [r"\b(bug|issue|problem|error)\b", r"(not|isn\'t)\s+working"],
        },
        UserIntent.COMPARE: {
            "keywords": ["compare", "versus", "vs", "difference", "better", "krahaso", "ndryshimi"],
    ...
    async def classify(self, message: str, context: Optional[Dict[str, Any]] = None, use_llm: bool = True) -> IntentClassificationResult:
        """
        ...
        if use_llm and (not intents or intents[0].confidence < 0.6):
            ...
            if llm_intent:
                ...
                if existing:
                    ...
                    intents.insert(0, IntentResult(intent=llm_intent, confidence=0.75, keywords=["llm_detected"]))
        ...
        # Default to unknown
        if not intents:
            intents.append(IntentResult(intent=UserIntent.UNKNOWN, confidence=0.5, keywords=[]))
    ...
    def _classify_intents(self, message: str) -> List[IntentResult]:
        ...
        for intent, config in self.INTENT_PATTERNS.items():
            ...
            if score > 0:
                results.append(IntentResult(intent=intent, confidence=min(0.95, score), keywords=matched_keywords))
    ...
    def _extract_slots(self, message: str) -> List[ExtractedSlot]:
        ...
        for slot_type, pattern in self.SLOT_PATTERNS.items():
            ...
            for match in matches:
                slots.append(ExtractedSlot(name=slot_type, value=match, type=slot_type))
