"""
CLX-PUBLISHER - Kloud Publishing Module
===========================================

Auto-publish engine for Kloud Content Factory.
...
class Platform(str, Enum):
    ...
    SUBSTACK = "substack"
    TWITTER = "twitter"  # X
    KLOUD_BLOG = "kloud"  # kameleon.life/blog
    GITHUB_PAGES = "github"
    DEV_TO = "devto"
    RSS = "rss"
    WEBHOOK = "webhook"
...
class ContentFormatter:
    ...
    @staticmethod
    def to_linkedin(
        doc: EAPDocument, format_type: ContentFormat = ContentFormat.FULL_ARTICLE
    ) -> str:
        """Format for LinkedIn"""
        ...
        content += "\n#Kloud #AI #Governance #Analysis"

        # LinkedIn limit: 3000 chars
        if len(content) > 3000:
            content = content[:2950] + "...\n\n[Read full analysis on kameleon.life]"
    ...
    def to_twitter_thread(doc: EAPDocument) -> List[str]:
        ...
        # Tweet 7: CTA
        tweet7 = f"📖 Full analysis: kameleon.life/analysis/{doc.id}\n\n"
        tweet7 += "#AI #Governance #Analysis #Kloud"
    ...
    def to_substack(doc: EAPDocument) -> str:
        ...
        content += "\n---\n\n"
        content += "*This analysis was generated by Kloud Cloud.*\n"
        content += "*Subscribe for more insights: kameleon.life*\n"
...
published: true
tags: {", ".join(tags)}
 canonical_url: https://kameleon.life/analysis/{doc.id}
 cover_image: https://kameleon.life/images/analysis-cover.png
---
...
"""
    ...
    def to_rss_item(doc: EAPDocument) -> Dict:
        ...
        return {
            ...
            "link": f"https://kameleon.life/analysis/{doc.id}",
            "guid": doc.id,
    ...
    def to_teaser(doc: EAPDocument, max_length: int = 300) -> str:
        ...
        teaser += f"...\n\n🔗 Read more: kameleon.life/analysis/{doc.id}"
        return teaser
...
class BasePlatformPublisher:
    ...
    async def publish(
        self, doc: EAPDocument, format_type: ContentFormat
    ) -> PublishResult:
    ...
    async def publish(
        ...
        try:
            ...
            return PublishResult(
                ...
                platform=self.platform,
                status=PublishStatus.PUBLISHED,
                url=f"https://twitter.com/kloud/status/{result_id}",
                published_at=self.last_publish,
    ...
    async def publish(
        ...
        try:
            ...
            return PublishResult(
                ...
                platform=self.platform,
                status=PublishStatus.PUBLISHED,
                url=f"https://medium.com/@kloud/{result_id}",
                published_at=self.last_publish,
                metadata={"content_length": len(content)},
            )
        except Exception as e:
    ...
    async def publish(
        ...
        try:
            ...
            return PublishResult(
                ...
                platform=self.platform,
                status=PublishStatus.PUBLISHED,
                url=f"https://kloud.substack.com/p/{result_id}",
                published_at=self.last_publish,
                metadata={"content_length": len(content)},
            )
        except Exception as e:
...
class KloudBlogPublisher(BasePlatformPublisher):
    """Kloud Blog publisher (internal)"""

    platform = Platform.KLOUD_BLOG

    async def publish(
        self, doc: EAPDocument, format_type: ContentFormat
    ) -> PublishResult:
        """Publish to Kloud blog"""
        ...
        try:
            ...
            return PublishResult(
                ...
                platform=self.platform,
                status=PublishStatus.PUBLISHED,
                url=f"https://kameleon.life/analysis/{doc.id}",
                published_at=self.last_publish,
...
class CLXPublisher:
    """
    CLX-Publisher - Kloud Publishing Engine
    ...
    def _init_default_configs(self) -> None:
        ...
        for platform in Platform:
            self._configs[platform] = PublishConfig(platform=platform, enabled=False)

        # Enable local publishing by default
        self._configs[Platform.KLOUD_BLOG].enabled = True
    ...
    def configure_platform(
        ...
        **kwargs,
        ...
        config = PublishConfig(
            ...
            or os.getenv(f"{platform.value.upper()}_ACCESS_TOKEN"),
        ...
        publisher_map = {
            ...
            Platform.KLOUD_BLOG: KloudBlogPublisher,
    ...
    def assess_quality(self, doc: EAPDocument) -> Dict[str, Any]:
        ...
        if (
            doc.proposi.implementation_steps
            and len(doc.proposi.implementation_steps) >= 3
        ):
            score += 0.10
        else:
    ...
    def select_platforms(self, doc: EAPDocument, quality: Dict) -> List[Platform]:
        ...
        # Always publish to Kloud blog if enabled
        if self._configs[Platform.KLOUD_BLOG].enabled:
            platforms.append(Platform.KLOUD_BLOG)
        ...
        if self._configs[Platform.DEV_TO].enabled:
            if any(
                tag in doc.tags for tag in ["ai", "tech", "architecture", "engineering"]
            ):
    ...
    async def publish(
        ...
        for platform in platforms:
            if platform not in self._publishers:
                ...
                if platform == Platform.KLOUD_BLOG:
                    self._publishers[platform] = KloudBlogPublisher(
            ...
            try:
                ...
                if result.status == PublishStatus.PUBLISHED:
                    ...
                    logger.warning(
                        f"❌ Failed to publish to {platform.value}: {result.error}"
                    )
    ...
    async def publish_with_quality_check(
        self, doc: EAPDocument, force: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "successful": sum(
                1 for r in results if r.status == PublishStatus.PUBLISHED
            ),
    ...
    async def process_queue(self, batch_size: int = 5) -> List[PublishResult]:
        ...
        sorted_queue = sorted(
            self._queue, key=lambda x: priority_order.get(x.priority, 2)
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_failed": len(
                [r for r in self._publish_history if r.status == PublishStatus.FAILED]
            ),
...
async def publish_eap(
    doc: EAPDocument, platforms: Optional[List[str]] = None
) -> List[Dict]:
            ...
            def __post_init__(self):
                if self.structural_gaps is None:
                    self.structural_gaps = [
                        {
                            "type": "accountability",
                            "description": "Missing oversight",
                            "severity": "major",
                            "missing": "Audit mechanism",
                        },
                        {
                            "type": "sovereignty",
