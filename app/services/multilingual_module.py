...
class RealMultilingualService:
    ...
    def translate_text(
        ...
        if (
            cached_result
            and datetime.now() - cached_result["timestamp"] < self.cache_ttl
        ):
        ...
        if self.aws_translate and source_language != "auto":
            try:
                ...
                logger.info(
                    f"Translation completed via AWS Translate: {source_language} -> {target_language}"
                )
            except Exception as e:
        ...
        if not translation_result and self.google_translate:
            try:
                if source_language == "auto":
                    result = self.google_translate.translate(
                ...
                logger.info(
                    f"Translation completed via Google Translate: {translation_result.source_language} -> {target_language}"
                )
            except Exception as e:
        ...
        if not translation_result:
            try:
                ...
                logger.info(
                    f"Translation completed via Deep Translator: {source_language} -> {target_language}"
                )
            except Exception as e:
    ...
    def speech_to_text(
        ...
        try:
            ...
            return SpeechRecognitionResult(
                text=text, language=language, confidence=confidence, duration=duration
            )
    ...
    async def text_to_speech(
        self, text: str, language: str = "en", output_file: Optional[str] = None
    ) -> str:
        """
    ...
    def batch_translate(
        ...
        for text in texts:
            try:
                ...
                logger.error(
                    f"Batch translation failed for text: {text[:50]}... Error: {e}"
                )
                # Return explicit failure instead of pretending original text is translated.
                results.append(
                    TranslationResult(
                        source_text=text,
                        translated_text="",
